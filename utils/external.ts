...
export const getDvolIndexDeribit = async (deribitBaseUrl: string | undefined) => {
    ...
    if(!deribitBaseUrl)
        ...
        const response = await fetch(`${deribitBaseUrl}/api/v2/public/get_volatility_index_data?currency=ETH&end_timestamp=${endPeriodTimestamp}&resolution=1D&start_timestamp=${startPeriodTimestamp}`);
